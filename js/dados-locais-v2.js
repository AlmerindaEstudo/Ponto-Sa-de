...
medicamentos: {
  "Geral": [
    { nome: "Metformina 500mg", status: true },
    { nome: "Losartana 50mg", status: true }
  ]
},
produtos: {
  "Geral": [
    { nome: "Fraldas Geriátricas", status: true },
    { nome: "Absorvente", status: true }
  ]
}
...
// --- APENAS UMA VERSÃO DA FUNÇÃO DE CARREGAR ---
function carregarLocaisPersistidos(listaPadrao) {
  try {
    ...
    return listaPadrao.map((local) => {
      const editado = porId.get(local.id);
      if (!editado) return local;

      // Esta linha unifica tudo (telefone, medicos, vacinas, medicamentos, produtos)
...
// Mantenha apenas esta função de salvar também
function salvarLocaisPersistidos() {
  try {
    // Apenas salva o estado atual do array locaisDeSaude
    localStorage.setItem(STORAGE_LOCAIS_EDITADOS, JSON.stringify(locaisDeSaude));
  } catch (error) {
